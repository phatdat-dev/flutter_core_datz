...
class OdooWebSocketService {
  ...
  OdooWebSocketService() {
    ...
    final uri = Uri.parse(
      url,
    ).replace(scheme: url.contains("https") ? "wss" : "ws");
    channel = IOWebSocketChannel(
      WebSocket.connect(uri.toString(), headers: {"Origin": baseUrl}),
    );
    channel.sink.add(
      jsonEncode({"Origin": GetIt.instance<DioNetworkService>().baseUrl}),
    );
  }
